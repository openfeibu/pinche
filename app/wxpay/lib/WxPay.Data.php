	...
	**/
		...
		foreach ($this->values as $k => $v)
		{
			...
			if($k != "sign" && $v != "" && !is_array($v)){
				...
				if(method_exists($this, 'GetAppid')) {
					if($this -> GetAppid() == \app\core\XConfig::get('config_pay_wx_appid')) {
						$k  = strtolower($k);
					}
				} else {
					$appid = $this -> values['appid'];
					if($appid == \app\core\XConfig::get('config_pay_wx_appid')) {
						$k  = strtolower($k);
					}
				}
				//JSAPI支付不需要转换成小写
	...
	public function MakeSign()
	{
		...
		if(method_exists($this, 'GetAppid')) {
			//APP支付
			if($this -> GetAppid() == \app\core\XConfig::get('config_pay_wx_appid')) {
				$string = $string . "&key=" . \app\core\XConfig::get('config_pay_wx_key');
			}
			//JSAPI支付
			if($this -> GetAppid() == \app\core\XConfig::get('config_pay_wx_js_appid')) {
				$string = $string . "&key=" . \app\core\XConfig::get('config_pay_wx_js_key');
			}
			//小程序支付
            if($this -> GetAppid() == \app\core\XConfig::get('config_pay_wx_small_appid')) {
                $string = $string . "&key=" . \app\core\XConfig::get('config_pay_wx_small_key');
            }
		} else {
			$appid = $this -> values['appid'];
			//APP支付
			...
			//JSAPI支付
			if($appid == \app\core\XConfig::get('config_pay_wx_js_appid')) {
				$string = $string . "&key=" . \app\core\XConfig::get('config_pay_wx_js_key');
			}
			//小程序支付
            if($appid == \app\core\XConfig::get('config_pay_wx_small_appid')) {
                $string = $string . "&key=" . \app\core\XConfig::get('config_pay_wx_small_key');
            }
		}
		//签名步骤三：MD5加密
	...
	public function SetPaySource($pay_source) {
	    return $this -> values['pay_source'] = $pay_source;
    }
...
{	
	...
	**/
	...
	{
		$this->values['spbill_create_ip'] = "192.168.1.1";
	}
	/**
