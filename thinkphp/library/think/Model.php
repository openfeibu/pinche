...
abstract class Model implements \JsonSerializable, \ArrayAccess
{
    ...
    {
        if ($name == $this->getPk() && $this->pk_auto_increase) {
            $this->isUpdate(true);
        }
...
//                throw new InvalidArgumentException('property not exists:' . $this->class . '->' . $name);
    ...
    public function toArray()
    {
        ...
        if (!empty($this->extras)) {
            $item = array_merge($item, $this->extras);
        }
    ...
    {
        //修改点  如果extras里有先取extras里的值
        if (array_key_exists($name, $this->extras)) return $this->extras[$name];
    ...
    //默认主键自动递增的
    protected $pk_auto_increase = true;
    /**
     * 添加附加信息
     * @param $key
     * @param $val
    ...
    public function putExtras($data, $prefix = "")
    {
        if ($prefix) {
            $prefix = $prefix . "_";
        }
        ...
        foreach ($data as $key => $value) {
            $this->putExtra($prefix . $key, $value);
        }
    }
     ...
     * @param array $keys
     */
    public function removeExtra($keys = [])
    {
        if (is_string($keys)) {
            $keys = [$keys];
        }
