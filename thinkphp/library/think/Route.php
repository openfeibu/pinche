...
class Route
{
    ...
    public static function parseUrl($url, $depr = '/', $autoSearch = false)
    {
        ...
        if (isset($path)) {
            // 解析模块
            $level=Config::get("module_level");
            if(!$level){
                $level=1;
            }
            $path_level=array_slice($path,0,(int)$level);
            $multi_module=$path_level[0];
            for ($i=1;$i<count($path_level);$i++){
                $multi_module.="/".$path_level[$i];
            }
            //修改点 支持模块多层级
            $module = Config::get('app_multi_module') ? $multi_module : null;
            $path=array_slice($path,(int)$level);
            if ($autoSearch) {
                // 自动搜索控制器
