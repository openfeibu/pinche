    ...
    public static function run(Request $request = null)
    {
        ...
        if ($data instanceof Response) {
            ...
            if (Config::get("app_trace") && isset($_GET['trace']) ? $_GET['trace'] : false) {
                if ($type != "html") {
                    $html = "<html><body>返回结果:<pre id='result'></pre><script>var result=" . json_encode($data) . "; result=JSON.stringify(result, null, 2);document.getElementById(\"result\").textContent=result;</script></body></html>";
                ...
                if ($class) {
                    ...
                    if ($bind instanceof $className) {
                        ...
                        if(method_exists($className, 'instance')){
                            //有单例方法使用单例
                            $args[]=$className::instance();

                        }elseif (class_exists("rap\\Ioc")){
                            //使用ioc容器
                            $args[]=\rap\Ioc::get($className);
                        }else{
                            //直接new
                            $args[]=new $className();
                        }
...
//                  throw new \InvalidArgumentException('method param miss:' . $name);
    ...
    public static function module($result, $config, $convert = null)
    {
        ...
        try {
            ...
            //修改点 触发 rap的事件
            $actionEventName=$module.".".$controller.".".$action;
            if (Config::get('url_param_type')) {
                $vars = Request::instance()->route();
            } else {
                $vars = Request::instance()->param();
            }
            if(class_exists("rap\\Aop\\EventHook")){
                \rap\Aop\EventHook::trigger("action_begin:".$actionEventName,$vars);
            }

            $data = self::invokeMethod($call,$vars);
            //修改点 出发控制器编织的结束
            if (class_exists("rap\\Aop\\CtrWave")) {
                $data = \rap\Aop\CtrWave::instance()->triggerAfter($data);
            }
            //修改点 触发rap的事件
            if(class_exists("rap\\Aop\\EventHook")){
                \rap\Aop\EventHook::trigger("action_end:".$actionEventName ,$data);
            }
